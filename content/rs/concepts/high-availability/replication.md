...
which is constantly synchronized with the master shard. If the node with the master
shard fails, an automatic failover happens and the slave shard is promoted
to be the new master shard. When the node with the old master shard recovers, the old master shard becomes
